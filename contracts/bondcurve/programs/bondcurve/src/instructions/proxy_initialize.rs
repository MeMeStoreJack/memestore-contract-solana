...
pub struct ProxyInitialize<'info> {
    ...
    #[account(mut)]
    pub bonding_curve_wsol: Box<InterfaceAccount<'info, TokenAccount>>,
    #[account(
    ...
    #[account(
        mut,
        constraint = creator.key() == global.params.dex_bot,
    )]
...
impl<'info> ProxyInitialize<'info> {
    ...
    pub fn withdraw_token_reserve(&self) -> Result<()> {
        ...
        anchor_spl::token::transfer(cpi_ctx, self.bonding_curve.token_reserves)
    }
    ...
    pub fn withdraw_wsol(&self) -> Result<()> {
        let seeds = [
            ...
            &[self.bonding_curve.bump_seed],
        ];
        let binding = [&seeds[..]];
        let cpi_ctx = CpiContext::new_with_signer(
            self.token_program.to_account_info(),
            Transfer {
                from: self.bonding_curve_wsol.to_account_info(),
                to: self.user_wsol.to_account_info(),
                authority: self.bonding_curve.to_account_info(),
            },
            &binding,
        );
        anchor_spl::token::transfer(cpi_ctx, self.bonding_curve.sol_reserves)
    }
